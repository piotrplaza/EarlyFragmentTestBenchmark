...
#include "ogl/shaders/mandelbrot.hpp"
...
std::unique_ptr<Shaders::Programs::Mandelbrot> mandelbrotShader;
...
void RenderScene()
{
	const glm::vec4 clearColor = { 0.0f, 0.1f, 0.2f, 1.0f };
	glClearColor(clearColor.r, clearColor.g, clearColor.b, clearColor.a);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glUseProgram_proxy(mandelbrotShader->getProgramId());
...
void PrepareFrame()
{
	mandelbrotShader->mvp.setValue(mvp.getMVP(model.get()));
...
void ChangeWindowSize(glm::ivec2 size)
{
	...
	mvp.projection = glm::perspective(glm::radians(50.0f), (float)size.x / size.y, 0.1f, 10.0f);
